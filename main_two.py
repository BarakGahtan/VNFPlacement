...
def objective(trial, env_params, n_episodes=100, n_steps=100):
    ...
    learning_rate = trial.suggest_float('learning_rate', 1e-5, 1e-3, log=True) #1e-5, 1e-3, log=True
    batch_size = trial.suggest_categorical('batch_size', [128]) #[64, 128, 256]
    n_steps = trial.suggest_categorical('n_steps', [1024]) # [512, 1024, 2048]
    gamma = trial.suggest_float('gamma', 0.95, 0.999) # 0.95, 0.999
    gae_lambda = trial.suggest_float('gae_lambda', 0.9, 0.99) #0.9, 0.99
    clip_range = trial.suggest_float('clip_range', 0.1, 0.3) #0.1, 0.3
